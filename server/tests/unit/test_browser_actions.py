...
async def test_pick_completes_a_suggestion_picker_first_time(sites, fresh_browser):
    ...
    t0 = time.monotonic()
    r = await browser.run("act-pick", {"action": "pick", "ref": _ref("act-pick", "Abholort"), "text": "Musterstraße 12, Dreieich"})
    assert r.ok, r.text
    assert time.monotonic() - t0 < 8
    assert "Musterstraße 12, 63303 Dreieich" in r.step
    assert '"Abholort" [picker: use pick] = "Musterstraße 12, 63303 D' in r.text
    r = await browser.run("act-pick", {"action": "pick", "ref": _ref("act-pick", "Ziel"), "text": "Beispielschule"})
    assert r.ok and "Beispielschule, Breslauer" in r.step
...
async def test_pick_never_takes_the_location_or_map_rows(sites, fresh_browser):
    ...
    r = await browser.run("act-pick-2", {"action": "pick", "ref": _ref("act-pick-2", "Abholort"), "text": "Beispielweg 5"})
    assert r.ok and "Beispielweg 5, 63225 Langen" in r.step
...
async def test_pick_with_no_matching_row_lists_what_it_offers(sites, fresh_browser):
    ...
    assert not r.ok and "no row matches" in r.text and "Musterstraße 12" in r.text
