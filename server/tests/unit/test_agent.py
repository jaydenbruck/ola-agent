...
async def test_language_and_facts_in_prompt(make_agent):
    ...
    assert "Musterstraße 12" in system
...
async def test_remember_and_remind(make_agent, bus, memory):
    def script(messages, tools):
        ...
        return Reply(
            tool_calls=[
                call("remember", cid="m", fact="Mia wohnt in der Beispielweg 5, Langen."),
                call("remind_at", cid="r", when="in 0 minutes", text="Denk an Mia!"),
            ]
        )

    agent = make_agent(script)
    await agent.run_turn(T, "Merk dir: Mia wohnt in der Beispielweg 5, Langen. Erinner mich gleich.", [], "x")
    assert "Mia wohnt in der Beispielweg 5, Langen." in memory.facts()
    events = await drain(bus, T, lambda h: types(h).count("assistant.done") == 2)
    spoken = [e["text"] for e in events if e["type"] == "assistant.done"]
    assert spoken == ["Merk ich mir.", "Denk an Mia!"], "a due reminder waits for the turn's own acknowledgement"
