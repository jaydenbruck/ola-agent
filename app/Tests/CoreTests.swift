...
final class CoreTests: XCTestCase {
    ...
    func testConfirmationFromEventAndSnapshotThenAcknowledgementAndNextStep() throws {
        ...
        state.answered("pizza", confirmation: confirmation, answer: "yes")
        XCTAssertNil(state.jobs[0].confirmAnswer)
        let row = try JSONDecoder().decode(JobSnapshot.self, from: Data(#"{"job_id":"ride","title":"Ride","state":"confirm","confirm":{"title":"Uber","price":"18 €","detail":"Arrives in 4 minutes"}}"#.utf8))
        XCTAssertEqual(row.card.state, .needsYou)
